...
describe('WorkspaceMetricsService', () => {
  const rows = [
    {
      ...
      finishedAt: new Date('2026-08-01T10:02:00Z'),
      environment: { name: 'dev' },
      buildArtifact: { createdAt: new Date('2026-08-01T09:50:00Z') },
    },
    {
      kind: 'redeploy',
      status: 'failed',
      startedAt: new Date('2026-08-01T11:00:00Z'),
      finishedAt: new Date('2026-08-01T11:01:00Z'),
      environment: { name: 'dev' },
      buildArtifact: null,
    },
    {
      kind: 'rollback',
      status: 'succeeded',
      startedAt: new Date('2026-08-02T11:55:00Z'),
      finishedAt: new Date('2026-08-02T12:00:00Z'),
      environment: { name: 'prod' },
      buildArtifact: { createdAt: new Date('2026-08-02T00:00:00Z') },
    },
    {
      kind: 'promote',
      status: 'cancelled',
      startedAt: new Date('2026-08-02T13:00:00Z'),
      finishedAt: new Date('2026-08-02T13:00:30Z'),
      environment: { name: 'prod' },
      buildArtifact: null,
    },
  ];
  ...
  function setup(role: string | null = 'owner') {
    ...
    const workspaces = {
      ...
      can: jest.fn(
        (candidate: string, permission: string) =>
  ...
  it('creates a stable CSV with one total and daily rows', async () => {
    ...
    expect(csv).toContain(
      'total,,2026-08-01T00:00:00.000Z,2026-08-03T00:00:00.000Z,4,2,1,1,66.67,1,1,120,12',
    );
  ...
  it('returns 404 outside the workspace and 403 to a non-admin member', async () => {
    ...
    await expect(
      setup('member').service.export('member-1', 'workspace-1', { format: 'json' }),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('rejects reversed and excessively wide periods before reading operations', async () => {
    await expect(
      setup().service.export('owner-1', 'workspace-1', {
        from: '2026-08-03',
        to: '2026-08-01',
        format: 'json',
      }),

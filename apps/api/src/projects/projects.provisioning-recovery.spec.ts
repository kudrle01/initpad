...
const effect = (key: string, kind: string, metadata: unknown = null) => ({
  key, kind, status: 'applied', metadata, error: null, createdAt: new Date(),
  appliedAt: new Date(), compensatedAt: null,
});
...
describe('ProjectsService provisioning recovery', () => {
  it('claims a safe retry and starts a linked next attempt', async () => {
    const operation = {
      ...
      kind: 'create', status: 'failed', requestedById: 'u1',
      request: { name: 'api', templateId: 'node-api' }, retryOfId: null,
      attempt: 2, effects: [],
    };
    ...
    expect(create).toHaveBeenCalledWith(
      ...
      'u1',
      'ws1',
      { retryOfId: 'op1', attempt: 3 },
    );
  });

  it('restores import collaborators and secrets before deleting the recovery record', async () => {
    const operation = {
      id: 'op1', workspaceId: 'ws1', projectId: 'p1', projectName: 'api',
      kind: 'import', status: 'failed', requestedById: 'u1', request: {},
    ...
    expect(scm.restoreCollaboratorAccess).toHaveBeenCalledWith(
      expect.objectContaining({ fullName: 'acme/api' }), 'bob', 'pull',
    );
  ...
  it('keeps the project when idempotent cleanup cannot reach the provider', async () => {
    const operation = {
      id: 'op1', workspaceId: 'ws1', projectId: 'p1', projectName: 'api',
      kind: 'import', status: 'interrupted', requestedById: 'u1', request: {},
      retryOfId: null, attempt: 1,
      effects: [effect('project:record', 'project'), effect('secrets:initpad', 'secrets')],
    };
    const { service, prisma, provisioning } = build(operation, {
      removeRepoSecrets: jest.fn(async () => { throw new Error('provider offline'); }),
    });
    ...
    expect(provisioning.compensationFailed).toHaveBeenCalledWith(
      'op1', 'secrets:initpad', 'provider offline',
    );
  });
  ...
  it('can remove an orphaned newly-created repository from journal metadata', async () => {
    const operation = {
      id: 'op1', workspaceId: 'ws1', projectId: null, projectName: 'api',
      kind: 'create', status: 'interrupted', requestedById: 'u1', request: {},
      retryOfId: null, attempt: 1,
      effects: [effect('repository:create', 'repository', {
        provider: 'github', repositoryId: '101', owner: 'acme', name: 'api',
        fullName: 'acme/api', defaultBranch: 'main', repoUrl: 'https://github.com/acme/api',

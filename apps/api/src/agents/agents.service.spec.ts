...
function setup(role: string | null = 'owner') {
  ...
  const workspaces = {
    ...
    can: jest.fn(
      (current: string, permission: string) =>
...
describe('AgentsService trust bootstrap', () => {
  ...
  it('issues a short-lived enrollment secret but persists only its hash', async () => {
    ...
    expect(call.create.enrollmentExpiresAt).toEqual(new Date(NOW.getTime() + 15 * 60_000));
  ...
  it('accepts an authenticated heartbeat and stores only bounded Docker telemetry', async () => {
    ...
    await expect(
      service.heartbeat(`Bearer ${credential}`, {
        version: '0.1.0',
        protocolVersion: 1,
        docker: {
          engineVersion: '27.5.1',
  ...
  it('stages an overdue credential for a compatible Agent without revoking the active one', async () => {
    ...
    expect(rotationWrite.where).toEqual(
      expect.objectContaining({
        id: 'agent-1',
        credentialHash: hashToken(credential),
        credentialGeneration: 3,
  ...
  it('rejects malformed, unknown and concurrently revoked Agent credentials', async () => {
    ...
    await expect(
      malformed.service.heartbeat('Basic nope', {
        version: '0.1.0',
        protocolVersion: 1,
        docker: {
          engineVersion: '27.5.1',
    ...
    await expect(
      unknown.service.heartbeat(`Bearer ${credential}`, {
        version: '0.1.0',
        protocolVersion: 1,
        docker: {
          engineVersion: '27.5.1',
    ...
    const revoked = setup();
    revoked.prisma.agent.findFirst.mockResolvedValue(
      agentRow({
        credentialHash: hashToken(credential),
      }),
    );
    ...
    await expect(
      revoked.service.heartbeat(`Bearer ${credential}`, {
        version: '0.1.0',
        protocolVersion: 1,
        docker: {
          engineVersion: '27.5.1',
  ...
  it('invalidates the previous credential when a new credential generation is enrolled', async () => {
    ...
    await expect(
      previous.service.authenticateCredential(`Bearer ${previousCredential}`),
    ).rejects.toBeInstanceOf(UnauthorizedException);
    ...
    const current = setup();
    current.prisma.agent.findFirst.mockResolvedValue(
      agentRow({
    ...
    await expect(
      current.service.authenticateCredential(`Bearer ${currentCredential}`),
  ...
  it('disables the identity without deleting the physical target', async () => {
    ...
    prisma.agent.findUnique.mockResolvedValue(
      agentRow({ credentialHash: hashToken('credential') }),
    );
